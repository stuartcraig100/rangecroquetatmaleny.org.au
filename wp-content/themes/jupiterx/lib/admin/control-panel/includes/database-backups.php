...
class JupiterX_Control_Panel_Database_Manager {
	...
	public function backup_db() {
		...
		if ( ! $dumped ) {
			$this->errors->add( 'can_not_create_backup_db_file', __( 'Can not create backup db file.', 'jupiterx' ) );
	...
	public function backup_media_records() {
		...
		if ( ! $dumped ) {
			$this->errors->add( 'can_not_create_media_records_backup_db_file', __( 'Can not create media records backup file.', 'jupiterx' ) );
	...
	public function restore_latest_db() {
		...
		if ( preg_match( $regExp, $latest_backup_file['full_path'], $matches ) ) {
			if ( 'zip' == $matches[3] ) {
				...
				if ( ! $unzipfile ) {
					$this->errors->add( 'error_unzipping_backup_file', __( 'There was an error unzipping the backup file.', 'jupiterx' ) );
			...
			$this->errors->add( 'invalid_backup_file_type', __( 'Invalid backup file.', 'jupiterx' ) );
		...
		if ( ! $this->jupiterx_filesystem->exists( $database_sql_file ) || ! $this->jupiterx_filesystem->is_readable( $database_sql_file ) ) {
			$this->errors->add( 'backup_file_is_not_exists_or_readable', __( 'The backup file is not exists or not readable.', 'jupiterx' ) );
	...
	public function restore_media_records() {
		...
		if ( ! isset( $latest_backup['full_path'] ) || ! $this->jupiterx_filesystem->exists( $latest_backup['full_path'] ) || ! $this->jupiterx_filesystem->is_readable( $latest_backup['full_path'] ) ) {
			$this->errors->add( 'media_records_backup_not_exists_or_readable', __( 'Media records backup file is not exists or not readable', 'jupiterx' ) );
