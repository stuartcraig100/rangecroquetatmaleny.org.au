...
function jupiterx_post_image() {
	...
	if ( ! jupiterx_post_element_enabled( 'featured_image' ) ) {
		return false;
	}
	...
	if ( $edit ) {
		...
		if ( empty( $edit_args ) ) {
			$image = jupiterx_get_post_attachment( $post->ID, 'large' );
		} else {
			$image = jupiterx_edit_post_attachment( $post->ID, $edit_args );
		}
	...
	jupiterx_open_markup_e( 'jupiterx_post_image', 'div', array( 'class' => 'jupiterx-post-image' ) );
...
// Filter.
jupiterx_add_smart_action( 'the_content_more_link', 'jupiterx_post_more_link' );
/**
 ...
 */
function jupiterx_post_more_link() {
	global $post;
