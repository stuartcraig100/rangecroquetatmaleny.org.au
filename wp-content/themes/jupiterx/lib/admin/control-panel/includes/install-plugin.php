...
class JupiterX_Control_Panel_Plugin_Manager {
	...
	public function __construct( $ajax_mode = true, $system_under_test = false ) {
		...
		if ( $this->get_ajax_mode() == true ) {
			...
			add_action( 'wp_ajax_abb_lazy_load_plugin_list', array( &$this, 'jupiterx_plugin_list_handler' ) );
			add_action( 'wp_ajax_abb_install_plugin', array( &$this, 'jupiterx_install_handler' ) );
			add_action( 'wp_ajax_abb_update_plugin', array( &$this, 'jupiterx_update_handler' ) );
			add_action( 'wp_ajax_abb_remove_plugin', array( &$this, 'jupiterx_remove_handler' ) );
		}
	}
	 ...
	 */
	public function jupiterx_install_handler() {
	 ...
	 * @return bool will return boolean status of action , all message is setted to $this->message()
	 */
	public function jupiterx_remove_handler() {
		$slug_name = ( ! empty( $_POST['abb_controlpanel_plugin_slug'] ) ? $_POST['abb_controlpanel_plugin_slug'] : null);
		$this->set_plugin_slug( $slug_name );
	 ...
	 * @return bool will return boolean status of action , all message is setted to $this->message()
	 */
	public function jupiterx_update_handler() {
		$slug_name = ( ! empty( $_POST['abb_controlpanel_plugin_slug'] ) ? $_POST['abb_controlpanel_plugin_slug'] : null);
		$this->set_plugin_slug( $slug_name );
		$this->update_plugin();
	}
	 ...
	 * @return bool will return boolean status of action , all message is setted to $this->message()
	 */
	public function jupiterx_plugin_list_handler() {
		try {
			$this->set_lock( 'jupiterx_plugin_list_handler' , true );
			$response = $this->list_of_installed_plugin();
			$this->set_lock( 'jupiterx_plugin_list_handler' , false );
			if ( $response == false ) {
				throw new Exception( $this->get_response_message() );
			}
			$installed_plugin = $this->get_response_data();
			$exclude_plugins = $this->array_column( $installed_plugin, 'slug' );
	...
	public function install() {
		...
		try {
			...
			if ( is_array( $api_response ) == false || count( $api_response ) < 1 ) {
				$message = 'The plugin ({param}) you are looking for does not exist.' ;
			...
			JupiterX_Control_Panel_Helpers::upload_from_url( $this->get_plugin_remote_url(), $this->get_plugin_remote_file_name(), $this->get_plugins_dir() );

			// Unzip IT
			$zip_path = $this->get_plugins_dir() . $this->get_plugin_remote_file_name();
			JupiterX_Control_Panel_Helpers::un_zip( $zip_path, $this->get_plugins_dir() );
			...
			// Remove ZIP file
			JupiterX_Control_Panel_Helpers::delete_file_and_dir( $zip_path );
	...
	public function list_of_installed_plugin() {
		try {
			$list_of_plugins = $this->plugins_custom_api( 0 , 0 , array( 'slug', 'basename', 'version', 'name', 'desc', 'img_url', 'required' ) );
	...
	public function precheck() {
		...
		if ( ! $jupiterx_filesystem->is_writable( $this->get_plugins_dir() ) ) {
	...
	public function remove_plugin( $plugin_slug ) {
		...
		try {
			...
			if ( strlen( str_replace( $this->get_plugins_dir(), '', $plugin_full_path ) ) > 2 ) {
				if ( ! $jupiterx_filesystem->is_writable( $plugin_base_directory ) ) {
					throw new Exception( 'Plugin directory is not writable  - RPPM01x01.' );
				}
				if ( JupiterX_Control_Panel_Helpers::delete_file_and_dir( $plugin_base_directory ) ) {
				...
				if ( JupiterX_Control_Panel_Helpers::delete_file_and_dir( $plugin_full_path ) ) {
					$this->message( 'Plugin successfully Removed.', true );
					return true;
				} else {
	...
	public function message( $message, $status = true, $data = null ) {
		...
		if ( $this->get_lock() == true && $this->get_system_under_test() == false ) {
			...
			$response['message'] = jupiterx_logic_message_helper( 'plugin-management' , $message , 'sys_msg' );
			$this->set_response( $response );
			return true;
