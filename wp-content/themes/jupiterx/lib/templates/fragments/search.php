...
add_filter( 'the_content', 'jupiterx_modify_search_page_loop_content' );
/**
 * Modify the search page loop content.
 *
 * @since 1.0.0
 *
 ...
 */
function jupiterx_modify_search_page_loop_content( $content ) {
	if ( has_excerpt() ) {
		$content = '<p>' . get_the_excerpt() . '</p>';
	} else {
		$content = '<p>' . wp_trim_words( strip_shortcodes( $content ), 30, '...' ) . '</p>';
	}
