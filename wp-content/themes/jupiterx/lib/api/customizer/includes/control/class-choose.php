			...
			<div class="jupiterx-choose-control-buttons">
				<# if ( data.multiple ) { #>
					<# _.each( data.choices, function( choice, key ) { #>
						...
						<label class="jupiterx-choose-control-button jupiterx-choose-control-{{ ( choice.icon ) ? 'icon' : 'label' }}" for="{{ data.id }}-{{ key }}"><# if ( choice.icon ) { #><img src="<?php echo esc_url( JupiterX_Customizer_Utils::get_assets_url() ); ?>/img/{{ choice.icon }}.svg" /><# } else { #>{{ choice.label }}<# } #></label>
					<# } ) #>
