...
final class JupiterX_Customizer_Status {
	...
	public function render_settings_table() {
		...
		<table class="table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Customizer Settings Name', 'jupiterx' ); ?></th>
					<th><?php esc_html_e( 'Control Type', 'jupiterx' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php foreach ( JupiterX_Customizer::$settings as $id => $setting ) : ?>
					<?php if ( ! in_array( $setting['type'], $exclude_type, true ) ) : ?>
						<tr>
							<td><?php echo $id; // @codingStandardsIgnoreLine ?></td>
							<td><?php echo $setting['type']; // @codingStandardsIgnoreLine  ?></td>
						</tr>
					<?php endif; ?>
				<?php endforeach; ?>
			</tbody>
